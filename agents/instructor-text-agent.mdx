...
from overeasy.agents import InstructorTextAgent

# Load an image from a traffic camera
traffic_image = Image.open("traffic_image.png")

# Initialize the InstructorTextAgent with the TrafficAnalysisResponse model
agent = InstructorTextAgent(response_model=TrafficAnalysisResponse)

# Execute the agent to analyze the image
execution_node = agent.execute(traffic_image)

# Print the structured response
print(execution_node.data)
```